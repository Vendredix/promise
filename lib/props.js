    ...
    if (map instanceof Map) {
      ...
      for (const entry of map.entries()) {
        if (entry[1] instanceof Promise) {
          promises.push(utils.resolveSingleValue(Promise, entry[1], item => result.set(entry[0], item)));
        } else {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (item instanceof Promise) {
          promises.push(utils.resolveSingleValue(Promise, item, item => result[key] = item));
        }
      }
    }
